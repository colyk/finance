...
const TransactionList = props => {
	...
	return props.transactions.map(({ _id, title, categories, amount, type, updatedAt }) => (
		<tr key={_id} className={type === 'expense' ? 'expense-border' : type === 'income' ? 'income-border' : null}>
			<td>{title}</td>
			...
			<td>{amountWithCurrency(amount)}</td>
				...
				<div className="popover-container">
					<div className="card">
						...
						<div className="card-body">
							{amountWithCurrency(amount)} - {formatDate(updatedAt, 'DD.MM.YYYY')} ({formatDate(updatedAt, 'dddd')})
							</div>
...
const CategoryChip = ({ category }) => {
	...
	return <span className="chip" style={style}> {category.type}</span>
};
